...
## 排序规则（关键，2026-08-26 确立）
- **racconti/ 下所有带编号的文档必须添加 `modified:` 字段**（RFC2822 格式，如 `Fri, 22 Aug 2026 00:00:00 +0000`），值为该目录精读项目启动日
- 同一目录所有文件 modified 相同 → 触发稳定排序 → 输入序 = 文件名序 = 01→N 正序
- 单目录日期推断：`git log --diff-filter=A -- <目录首篇文件>` 取首篇精读 commit 日期；不确定时统一用 2026-08-22
